...
def test_ssot_canonical_artifact_beats_related_supporting_note_on_source_of_truth_query(
    tmp_path,
):
    ...
    recall = store.recall(
        "what is the source of truth for BrainOS retrieval quality?",
        session_id="s1",
        limit=5,
    )
    ...
    assert (
        top["content"]
        == "SSOT: source of truth for BrainOS retrieval quality lives in retrieval-quality-contract-v1."
    )
