...
def test_continuation_guardrail_protects_current_restart_and_current_direction(
    tmp_path,
):
    ...
    restart = store.recall(
        "what is the current restart point for BrainOS?", session_id=session_id, limit=5
    )
    ...
    direction = store.recall(
        "what front is currently active in BrainOS?", session_id=session_id, limit=5
    )
