...
def test_real_sample_benchmark_pass(monkeypatch, tmp_path):
    ...
    assert all(
        item["episode_vector_mode"] == "sqlite_vec_episode_similarity"
        for item in benchmark_report
    )
    assert all(
        item["semantic_vector_mode"] == "sqlite_vec_semantic_similarity"
        for item in benchmark_report
    )
    store.close()
...
def test_real_sample_scores_explain_top_hit(monkeypatch, tmp_path):
    ...
    assert (
        "episode_vector" in top_episode["score_components"]
        or "fts_rank" in top_episode["score_components"]
    )
    assert "score_components" in top_semantic
    assert (
        "semantic_vector" in top_semantic["score_components"]
        or "name_match_rank" in top_semantic["score_components"]
    )
    store.close()
...
def test_real_sample_policy_version_case(monkeypatch, tmp_path):
    ...
    assert recall["scoring_policy_version"] == "retrieval-scoring-v1b"
    store.close()
...
def test_real_corpus_probe_aligns_to_available_session_sample(tmp_path):
    ...
    store.add_episode(
        ...
        content="BrainOS initialized with WAL and ledger",
        metadata={},
    )
    store.add_episode(
        session_id="session-a", content="Store a semantic fact", metadata={}
    )
