...
Returns a single episode with parsed `metadata`.
...
This is explicit preview logic, not automatic background consolidation.
...
- ledger causal link points back to the source episode id
...
Output:
- JSON preview of a semantic/procedural promotion candidate
...
Output:
- JSON result of explicit promotion
